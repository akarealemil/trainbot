...
class Admin(commands.Cog):
    ...
    @commands.command(hidden=True)
    async def smessage(self, ctx):
        """Sends a message to a user."""

        def check50(m):
            return ctx.author == m.author

        embedmain1 = discord.Embed(title="Send a message", description="Please enter a User ID / Channel ID to send a message to")
        mainembed1 = await ctx.channel.send(embed=embedmain1)

        try:
            user = await self.bot.wait_for('message', timeout=60.0, check=check50)
            user = user.content

        except asyncio.TimeoutError:
            embederror = discord.Embed(title="CANCELLED", description=f"You didn't enter an ID in time!", colour=discord.Colour.from_rgb(255,20,147))
            await ctx.channel.send(embed=embederror)
        else:

            embedmain2 = discord.Embed(title="Send a message", description="Please enter a message to send to the user")
            mainembed2 = await ctx.channel.send(embed=embedmain2)

            try:
                msg = await self.bot.wait_for('message', timeout=300.0, check=check50)
                msg = msg.content
                
            except asyncio.TimeoutError:
                embederror = discord.Embed(title="CANCELLED", description=f"You didn't enter a message in time!", colour=discord.Colour.from_rgb(255,20,147))
                await ctx.channel.send(embed=embederror)
            else:
#                await ctx.send(user)
#                await ctx.send(msg)
                ...
                replyembed = discord.Embed(title="Suggestion", description=msg)
                links = (
                ...
                replyembed.add_field(name="**Links**", value=links, inline=False)
